...
public class MovingBoardRepair : MonoBehaviour
{
    ...
    [SerializeField] bool back = false;  //현재 목적지를 찍고 되돌아 가는 중인지
    private bool movingOn = false;  //현재 움직이고 있는지. awakeStart가 false 때 필요

    private bool playerCheck = false;   //플레이어 캐릭터가 탑승했는지
    ...
    IEnumerator Move()
    {
        ...
        while (true)
        {
            ...
            else       //경유지에 도착한게 아니라면 이동 중이다.
            {
                ...
                transform.position = Vector3.MoveTowards(transform.position, Pos[cur], speed * Time.deltaTime); //목적지를 향해 이동한다.
                if (playerCheck)
                {
                    if (playerMove.dir.y < 0)  //플레이어가 점프 도중이 아닌 제대로 타고 있다면
                    {
                        playerCC.Move((transform.position - prevPos) *transform.lossyScale.x);    //이동 후 좌표에서 이동 직전 좌표를 뺀만큼 플레이어 캐릭터를 이동시킨다.
                                                                        //이러면 정확히 오브젝트와 같이 움직인다.
                        playerCC.Move(new Vector3(0, -2.0f * Time.deltaTime, 0) * transform.lossyScale.x);   //이걸 해주지 않으면 isGrounded가 제대로 활성화되지 않는다.
                        if (jumpInertia)    //만약 점프 관성이 true라면
                            jumpInertiaValue = ((Pos[cur] - transform.position).normalized * speed).y;  //deltatime을 곱하지 않은 그대로의 속도에서 y값만 취한다.
                                                                                                        //아래 OnTriggerExit()함수를 통해 플레이어가 점프로 나갔다면 이 값을 점프에 가감해 관성을 받게 한다.
                    }
                }
                yield return delay;
            }
    ...
    {
        if (col.CompareTag("T_Player"))   //플레이어의 발에 붙어있는 콜라이더
        {
            playerCheck = true;         //플레이어가 탑승
            if (!movingOn && playerMove.dir.y < 1) //현재 정지 상태이고 플레이어가 점프 중이 아니라면
            {
                movingOn = true;
                StartCoroutine("Move"); //출발
            }
        }
    }
    ...
    private void OnTriggerExit(Collider col)
    {
        ...
        {
            playerCheck = false;
            if (jumpInertia)    //점프 관성이 켜져있다면 위에서 구해둔 값을 가감한다.
            {
    ...
    private void DestroyWait()  //위에서 최종목적지에 도달하여 사라져야 할 때 호출되는 함수
    {
        this.gameObject.SetActive(false);   //재사용 해야하므로 비활성화만 시켜준다.
        Invoke("Respawn", respawnTime);     //재생성 대기시간만큼 Invoke함수로 대기시킨다.
    }
